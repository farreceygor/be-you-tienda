    ...
    <main class="max-w-2xl mx-auto px-4 mt-6">
      ...
      <div v-if="vistaActual === 'productos'" class="fade-in card-form shadow-xl bg-white rounded-3xl p-6">
        ...
        <form @submit.prevent="guardarProducto" class="space-y-5">
          ...
          <div>
            <label class="label-admin font-bold">Variantes / Fragancias</label>
            <input
              type="text"
              v-model="nuevoProducto.variantes"
              placeholder="Ej: Vainilla, Coco, Lavanda"
              class="input-admin"
            />
            <p class="text-[10px] text-gray-400 mt-1 italic">
              Separa con comas. Déjalo vacío si no tiene variantes.
            </p>
          </div>
...
const guardarProducto = async () => {
  ...
  try {
    ...
    const datosFinales = {
      ...
      imagen_url: url,
      variantes: nuevoProducto.value.variantes || ''
    };
